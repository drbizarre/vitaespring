...
class Members extends CI_Controller {
	...
	//required callback -- codeigniter forces these to be in the controller -- this function is private
	function _check_direct_exist_create($directo) {
		
		$directo = substr($directo, 2);
			echo $directo;
		$this->load->model('user');
		$result = $this->user->check_direct_exist($directo);
		print_r($result);
		if ($result == TRUE) {
			$this->form_validation->set_message('_check_direct_exist_create', 'Patrocinador "'.$directo.'" no existe. Revisa nuevamente el número.');
			return FALSE;
		} else {
			return TRUE;
		}
