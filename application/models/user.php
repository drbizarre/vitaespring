...
class User extends CI_Model {
	...
	function getMisAfiliados($afiliado = NULL) {
		$this->db->select('user.user_id, user.directo ,user.first_name, user.last_name, user.email, user.status, user.member_since, afiliados.tipo,  afiliados.id');
		$this->db->from('user');
		$this->db->join('afiliados', 'afiliados.user_id = user.user_id');		
		$this->db->where('afiliados.user_id', $afiliado);
		$this->db->order_by('user.user_id', 'asc');
		$query = $this->db->get();
		return $query->result();
	}	
	function countMisAfiliados($user_id) {
	...
	function getUserByAfiliado($id = NULL) {
		$this->db->where('id', $id);
		$query = $this->db->get('afiliados');
		return $query->row(); 
	}	
	...
	function setTipo($afiliado = NULL,$tipo = NULL) {
			$data = array('tipo' => $tipo);
			$this->db->where('id', $afiliado);
			$this->db->update('afiliados', $data); 
	}	
	function setTipoUser($user = NULL,$tipo = NULL) {
			$data = array('tipo' => $tipo);
	...
	function verify_and_get_user($username, $password) {
		$this->db->select('user_id, unidad,empresa,username, first_name,imagen, last_name, user_level,pais,estado,ciudad,telefono,email,cp,direccion');
		$this->db->where('username', $username);
	...
	function check_direct_exist($directo) {
		
		$this->db->select('user_id');
		$this->db->where('user_id', $directo);
	...
	function create_afiliado($new_afiliado_array) {
		$query = $this->db->insert('afiliados', $new_afiliado_array);

		if ($query == TRUE) {
			$data['is_true'] = TRUE;
			$data['message'] = 'A new affiliate has been successfully created.';
			$data['id'] = $this->db->insert_id();
			return $data;
		} else {
			$data['is_true'] = FALSE;
			$data['message'] = 'Failed to create affiliate! Please try again, if the problem persists, contact a site admin.';
			return $data;
		}
	}	
	//get users
	...
	function delete_afiliado($afiliado) {
		$this->db->where('id', $afiliado);
		$query = $this->db->delete('afiliados');
