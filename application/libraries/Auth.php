...
class Auth {
	...
	function create_user($page) {
        ...
        $this->CI->form_validation->set_rules('directo', 'Patrocinador', 'required|trim|min_length[4]|max_length[25]|xss_clean|numeric|callback__check_direct_exist_create[user.user_id]');
		...
		if ($this->CI->form_validation->run() == FALSE) {
			...
			if ($result['is_true'] == TRUE) {
				...
				$this->CI->session->set_flashdata('message', '<div class="alert alert-success">'.$result['message'].' <a href="'.site_url("backoffice").'">Login</a></div>');
